                ...
                child: Column(
                  children: <Widget>[
                    Icon(Icons.local_movies,
                        size: 95, color: Colors.deepPurple[600]),
...
//Botão Enviar
                    ...
                    RaisedButton(
                      ...
                      child: Text(
                          'Não é cadastrado? Clique aqui para se cadastrar'),
                      onPressed: () {
...
Future<Widget> checaLogin(
    BuildContext context, String username, String senha) async {
  ...
  if (documents.isNotEmpty) {
    if (documents[0].data['password'] == senha) {
      ...
      flag = 'Senha Incorreta! Tente outra vez.';
  } else
    flag =
        'Usúario Não Encontrado! Para se cadastrar, clique no botão "Não é cadastrado"';
  if (flag.isNotEmpty)
    return showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(content: Text(flag), actions: [
            FlatButton(
                onPressed: () {
